...
let logos: LogoOption[] = [{ id: 'default', name: 'INVIZION HOLDING', url: defaultLogo }];
...
function calculateRowAmount(qty: string, rate: string): string {
...
function currentLogo(): LogoOption {
            ...
            <section class="invoice-meta" aria-label="Invoice details">
              ...
              <div>
                ...
                <select data-field="currency">
                  ${(['USD', 'TRY', 'EUR'] as CurrencyCode[]).map((code) => `<option value="${code}" ${code === currency ? 'selected' : ''}>${code} (${currencySymbols[code]})</option>`).join('')}
                </select>
...
function updateCell(id: number, field: keyof InvoiceRow, value: string) {
  rows = rows.map((row) => {
...
function uploadLogo(fileInput: HTMLInputElement) {
...
document.addEventListener('click', (event) => {
  ...
  if (action === 'delete-row') deleteRow(Number(button.dataset.row));
});

document.addEventListener('change', (event) => {
  const target = event.target as HTMLInputElement | HTMLSelectElement;
  const rowId = target.dataset.row;
  const column = target.dataset.column as keyof InvoiceRow | undefined;
  ...
  switch (target.dataset.field) {
    ...
    case 'dueDate':
      ...
      break;
    case 'currency':
      currency = target.value as CurrencyCode;
      render();
      break;
    case 'vatRate':
